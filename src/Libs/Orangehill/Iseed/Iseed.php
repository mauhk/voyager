...
namespace TCG\Voyager\Libs\Orangehill\Iseed;
...
class Iseed
{
     ...
     * @param  array    $colsDontUpdate
     * @return bool
     * @throws TCG\Voyager\Libs\Orangehill\Iseed\TableNotFoundException
     */
    public function generateSeedUpsert($table, $colsDontUpdate = [])
{
        ...
        // Get the data
        $data = $this->getData($table, 0, null, null, 'ASC');
        ...
        $stub = $this->readStubFile($this->getStubPath());
        ...
        $seedContent = $this->populateStubUpsert(
            $className,
            $stub,
            $table,
            $dataArray,
    ...
    {
        return __DIR__ . DIRECTORY_SEPARATOR . 'stubs/' . config('iseed::config.stub_file');
    }
    ...
    public function populateStubUpsert($class, $stub, $table, $data, $chunkSize = null, $prerunEvent = null, $postrunEvent = null, $indexed = true, $colsDontUpdate = [])
    {
        $chunkSize = $chunkSize ?: config('iseed::config.chunk_size');
        ...
        foreach ($chunks as $chunk) {
            ...
            foreach ($colsDontUpdate as $col) {
                if (($key = array_search($col, $keys)) !== false) {
                    unset($keys[$key]);
                }
            }
            $keys = array_values($keys);
            $inserts .= sprintf(
                "DB::table('%s')->upsert(%s, %s, %s);",
                $table,
                $this->prettifyArray($chunk, $indexed),
                json_encode(array_shift($keys)),
                json_encode($keys)
            );
        }
